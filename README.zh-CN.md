...
组中都保持开启——这个开关隔离的是决策层 + 召回，而不是记忆的所有用途。）
这是*机制*层面的对比；它在测量上到底换来了什么，见下方
[评测方法论](#评测方法论)，包括它一无所获的地方。召回本身是纯结构指纹打分，
不是语义理解——它可能把两个形状相同、但根因毫不相关的失败配上同一条修复提示
（有实测记录：见 [`docs/KNOWN_LIMITATIONS.md`](docs/KNOWN_LIMITATIONS.md) L8）；
真正防止这种误配污染重试结果的，是代码生成模型把提示当建议、而不是当补丁来用。
...
- [`docs/eval/PHASE1_BIRD_ABLATION.md`](docs/eval/PHASE1_BIRD_ABLATION.md) ——
  ...
  系统）：20 个锁定用例 × 2 组 × 5 seed（200 次运行），记录字段以 SQL
  比较器为准，语料在运行前冻结
  （[`PHASE1_CORPUS.md`](docs/eval/PHASE1_CORPUS.md)）。success_rate 上为
  null（两组均 65.0%），tokens-per-solved 为 3.1 倍，且预注册的敏感性附录
  返回 **ASYMMETRIC**：内部评估器拒绝了 governor 组 80.8% 的比较器判定
...
- [`docs/eval/PHASE1_BIRD_ABLATION_R2.md`](docs/eval/PHASE1_BIRD_ABLATION_R2.md) ——
  ...
  相同的锁定语料和协议。敏感性附录：两组评估器 FN 均为 0.0%，**判定
...
- [`docs/eval/PHASE1_BIRD_ABLATION_R3.md`](docs/eval/PHASE1_BIRD_ABLATION_R3.md) ——
  ...
  **已发布运行时的记录**），相同的锁定语料和协议。null 第三次复现：
  ...
  再次 symmetric（两组评估器 FN 0.0%）。检测器本身在 200 次运行中记录
  **零次激活**，原始记录说明了原因：governor 组 31 次重试尝试中的 30 次是
  ...
  不含零，但锁定的用例级稳健性检查包含零（[-14.0, +4.0]），且两组在构造上
