...
每格格式为 **`governor / naive → Δ [95% CI] · 判决`** —— 把参与相减的两个原值和它们
的差摆在一起,`Δ = governor − naive`。前三行(success/first_try/recovery)的
governor、naive 单位为 %,Δ 单位为 pp;attempts 是次数;tokens 是个。

| 指标 | R1:gov / naive → Δ [95% CI] | R2:gov / naive → Δ [95% CI] | R3:gov / naive → Δ [95% CI] |
|---|---|---|---|
| **success_rate** | 65.0 / 65.0 → **+0.0** [−4.4, +4.4] · null | 61.0 / 61.0 → **+0.0** [−4.4, +4.4] · null | 61.0 / 62.0 → **−1.0** [−9.1, +7.1] · null |
| first_try_rate | 29.0 / 65.0 → **−36.0** [−38.8, −33.2] · 显著 | 58.0 / 61.0 → −3.0 [−10.1, +4.1] · null | 56.0 / 61.0 → −5.0 [−9.4, −0.6] · 显著* |
| recovery_rate | 50.8 / 0.0 → **+50.8** [+46.0, +55.5] · 显著 | 6.5 / 0.0 → +6.5 [−5.0, +18.0] · null | 11.4 / 2.5 → +8.9 [−2.7, +20.6] · null |
| attempts_per_case | 2.70 / 1.00 → +1.70 [+1.62, +1.78] · 显著 | 1.32 / 1.00 → +0.32 [+0.24, +0.40] · 显著 | 1.31 / 1.01 → +0.30 [+0.12, +0.48] · 显著 |
| tokens_per_solved | 14,449 / 4,594 → **+9,854** [+9.4k, +10.3k] · 显著 | 6,457 / 4,615 → +1,842 [+1.0k, +2.7k] · 显著 | 7,351 / 4,644 → +2,707 [+1.2k, +4.2k] · 显著 |
