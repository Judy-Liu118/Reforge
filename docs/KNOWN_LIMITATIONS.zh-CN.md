...
同一条边界已经用过三次：**只要 fixture server 也能触发它，它就是缺陷，不是被
推迟的决定**，因而不属于上面任何一张表。
...
**一个承诺了自己不做的事的参数是缺陷，不是被推迟的决定**。

`kill()` 之后没有 `wait()`（原「待办债务」第 2 行）已修复：`shutdown()` 在
SIGKILL 之后补上 `wait(timeout=1.0)`。它曾被列为「潜伏」，但这个判断本身是错的
—— 潜伏的原因不是 fixture 守规矩，而是 fixture **没有能力**不守规矩。修复同时给
`_mcp_test_server.py` 加了 `REFORGE_TEST_MCP_IGNORE_SIGTERM=1`，让这条路径可被
证伪。该测试在 Windows 上跳过（`kill()` 即 `terminate()`，且不存在僵尸），只在
Linux CI 上真正执行 —— 参见 L15 关于本地与 CI 跳过集合互补的记录。

脏行被丢弃却不计数（原「待办债务」第 5 行）已修复：`MCPClient` 现在计数并保留
最近 3 行样本，且把它作为后缀挂在超时与 EOF 的错误消息上。价值不在计数器本身，
而在**症状端**：一个把日志混进 stdout 的 server 此前和一个单纯很慢的 server
在错误消息上完全无法区分。同样地，fixture 加了
`REFORGE_TEST_MCP_STDOUT_NOISE=1` 才使它可证伪。
...
**本地那一行的采集条件 —— 它是数据的一部分，不是背景。** 该行是在 **Docker
Desktop 未启动**时采集的。那三条 docker 集成测试之所以成为它的独有跳过，全部
原因就是这一件事；与机器、操作系统或检出内容都无关。这个条件当时没有被记录，
是事后补写的（2026-08-07），而这件事本身正是本条目要说的：**"本地"不是一个
地点，而是一台机器在某一种状态下的一次读数。** 一次未写下采集条件的测量，会在
条件改变的那一刻悄悄变成一个关于错误对象的断言 —— 而条件确实改变了，见下。
...
| 缺口 | 方向 | 性质 | 可否弥合 |
|---|---|---|---|
| docker（3 条） | CI 有、本地无 | 环境**状态** —— 只是 Docker Desktop 没启动；把它开起来即可弥合，无需改代码 | 可以，且已经弥合 —— 但状态会翻回去，见下 |
| BIRD（1 条） | 本地有、CI 无 | 环境**内容** —— 2.1 GB 数据集，被 `.gitignore`（`data/`）排除，无法低成本送上 runner | 原则上可以，做一个小型 fixture 子集 |
| POSIX zombie reap（1 条） | CI 有、本地无 | 环境**平台** —— `test_kill_escalation_reaps_the_child` 断言 `shutdown()` 在 SIGKILL 之后完成 reap。Windows 上 `kill()` **就是** `terminate()`，TerminateProcess 无法被忽略，且不存在僵尸进程 | **不能** —— 被断言的那条路径在另一个平台上根本不存在 |

三者方向不同，无法用同一种方式解决。把"让跳过集合一致"当作一件事来做，正是
这张表存在的目的所要防止的错误。第三行把这一点说得比前两行更锋利：docker 是一个
可以启动的服务，BIRD 是一个原则上可以投送的文件，但再多的环境工作也无法让
Windows 上出现一个僵尸进程。它是本条目里第一个**由构造决定即永久存在**的不对称，
而一个永久的缺口无法靠"弥合"来管理 —— 只能靠"被看见"。

### 2026-08-07 重新测量：同一个幻觉，这次发生在本条目自己的数字上

本地在 **Docker Desktop 已启动**的状态下重跑，且代码树上还多了一条 POSIX-only
测试。本地跳过集合移动了四条：

- **−3** docker 集成测试 —— daemon 是起着的（`docker version` 报
  `linux/29.4.0`），这三条真的执行了而不是跳过
- **+1** `test_kill_escalation_reaps_the_child`，由 MCP 缺陷修复引入

本地现为 **5 条跳过，实测**，全量运行，exit 0：

```
1859 passed, 5 skipped in 95.06s (0:01:35)
```

SKIPPED 清单：2 条 LLM smoke、docker 哨兵、tavily、POSIX zombie reap。这组数字
与基线行**精确对得上**，这一点值得写出来，因为它是新数字能被当作"同一次测量"
而非"另一种形状的测量"来信任的唯一依据：collected 从 1860 → 1864（+4 条 MCP
测试），passed 从 1853 → 1859（+3 条现在真的执行的 docker 集成测试，+4 条新测试，
其中 −1 条在此处跳过）。

关于如何拿到那一行，附一笔，因为它花了三次运行：`pyproject.toml` 的 `addopts`
里已经有 `-q`，命令行再传一个 `-q` 就变成 `-qq`，而 `-qq` **会完全抑制最后那行
计数**。前两次运行只报出 exit 0 和一份 SKIPPED 清单，没有任何总数。正确用法是
`pytest reforge/tests -rs`，让 `addopts` 去提供 `-q`。

CI 那一侧是**推定，非实测** —— 这台机器上没有 `gh`，此后也没有读过任何一次 CI
运行。推导过程：基线的 7 条减去 Pillow 与 playwright（cb04992 把它们放进了
`[test]` extra），保留 BIRD，且不增加任何一条 —— 因为那条新的 POSIX 测试在
ubuntu 上是**跑**的。CI：**5 条，推定**。

| | 跳过 | 那边跳了、这边没跳的 |
|---|---|---|
| 本地（Windows，docker 已启动） | 5 —— 实测 | POSIX zombie reap |
| CI（ubuntu-latest） | 5 —— 推定 | BIRD |

**计数相同、独有集合互不相交 —— 第二次。** 第一次是三对三，两边都读作 7。这一次
是一对一，两边都读作 5。机制没有变化，且在上面"数字为什么掩盖了它"一节里已经
写明：计数在替换下不变。

新的东西是它**发生在哪里**。这次不是发生在某个不相干的套件上，而是发生在**本条目
自己的表格上**，就在写下这条警告与重读这条警告之间的那段时间里。一个被记录在案
的失效模式，若能在描述它的那份文档内部复现，那么"由人去读 SKIPPED 清单就足以
兜住"这个假设，已经被实例证伪了。

### 裁定：这条 POSIX-only 测试算不算第三处不对称

**算。触发条件已经成立。** 规则对设立它的人同样生效，所以这里记录推理过程，而不是
直接下断言。

**关于措辞。** 触发条件写的是"出现第三处跳过集合的不对称"，后面跟着"（第四个可选
依赖，或一条新的凭据门控测试）"。这条新测试两者都不是。但那个括号列举的是**它可能
以什么方式出现**，不是它成立的条件；起作用的是主句，而按本条目自己的定义，这就是
一处跳过集合的不对称 —— 一条测试因环境原因在一侧跑、在另一侧不跑。

**公平地陈述反方论证。** docker 现在开着，于是**瞬时活跃**的不对称是 BIRD 与 POSIX
两处，不是三处。据此可以主张阈值尚未达到。

**为什么这个论证不成立。** 两个理由。其一，docker 的弥合是**单日的一次状态观测**；
触发条件自己的第三条要求的是 Docker Desktop 变为"稳定可用"，而读一次
`docker version` 不构成稳定性。把它记作已关闭，恰恰就是反模式所禁止的那种推断 ——
把一次测量当成一种属性。其二，触发条件计的是不对称的**出现**，不是当前尚存的活跃
数量。若按活跃数量来读，它实际上永远无法触发：任何一处不对称都可能恰好在有人去看
的那天被掩盖掉。

**决定性的证据不是定义层面的。** 而是：上面那个失效模式确实在本条目自己的数字上
复现了；以及这处新的不对称是第一个**根本无法弥合**的。原来那两处之所以"尚可商榷"，
一部分原因正是各自都有一条通往关闭的路；而对一个平台缺口，"我们把它修掉，问题就
消失了"这条路并不存在。

**声明作者折扣。** 这处不对称，是由此刻援引该规则的同一次改动引入的。这构成从严
解读的理由，而不是从宽的理由 —— 让一条自设规则永不触发的最省事办法，就是在轮到
自己时判得宽松一点。此处明写出来，是为了让这种宽松（如果存在）可被看见。

**后果。** CI 侧的快照方案从"待答问题、值得做"变为**应当实施**。此处刻意不实施：
本条目自己已经写明它必须作为一次独立的、有意的改动来采纳，而把它夹带进一个关于
MCP 缺陷的提交里，就会在发出该指令的同一页上违反它。
...
- ~~出现第三处跳过集合的不对称（第四个可选依赖，或一条新的凭据门控测试）。两处
  ...
  **已于 2026-08-07 触发** —— 见上面的裁定一节。快照方案不再是待答问题，它已
  应当实施，且应作为独立改动实施（本条目自己的指令：不得夹带进不相关的提交）。
...
- ❌ 断言跳过的**条数**。它在替换下不变 —— 正是产生本条目的那种盲区，因为两边
  报的都是 7；随后它又在 5 上复现了一次。
- ❌ 把"让本地与 CI 跳过相同的东西"当作一件事来做。剩下的几个缺口方向不同，
  其中一个根本无法弥合（见上表）。
- ❌ 把本地那一行读作"本地"的属性。它是一台机器在某一种状态下的读数；daemon
  开着的那天，docker 那几条跳过就消失了。任何脱离采集条件被引用的本地测量，
  都是一个关于错误对象的断言。
